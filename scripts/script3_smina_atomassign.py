...
# parameters
_dir = f'/group/ag_cmb/scratch/sars-cov2/virtual_screening/'
runs = ['enamine']
receptors_list = np.loadtxt(_dir + 'receptor_list_run3.txt', dtype=str)

# set up basic receptor to get started with atomic indexing
receptor_base = mdtraj.load_pdb(_dir + f'structures/receptors_run3/{receptors_list[0]}.pdb')
...
# selection string may cause problems in script (not ipynb)
flexres_selection = '(sidechain or name C CA) and (mass > 2 or ((resname TRP and name HE1) or (resname LYS and name ' \
                    'HZ1 HZ2 HZ3) or  (resname GLN and name HE21 HE22))) and residue 44 45 180 183 206'
...
all_but_flexres_missing_hydros_selstr = '(mass < 2 and not ((resname TRP and name HE1) or (resname LYS and name HZ1 ' \
                                        'HZ2 HZ3) or (resname GLN and name HE21 HE22))) and residue 44 45 180 183 206' \
                                        ' and not name H HA'
...
for d_run in tqdm(runs):
    malfunctions = []
    ligands_list = np.loadtxt(_dir + f'ligand_list_run{d_run}.txt',
                              dtype=str)
    for receptor_name in tqdm(receptors_list[:4]):
        receptor_base = mdtraj.load_pdb(_dir + f'structures/receptors_run3/{receptor_name}.pdb')
        ...
        for ligand in tqdm(ligands_list):
            if os.path.exists(_dir + f'dockings_run{d_run}/{ligand}/{receptor_name}.pdbqt.pdb') and not os.path.exists(
                    _dir + f'dockings_run{d_run}/{ligand}/{receptor_name}_docked2.pdb'):
                try:
                    pose = mdtraj.load_pdb(_dir + f'dockings_run{d_run}/{ligand}/{receptor_name}.pdbqt.pdb',
                                           frame=0)

                    flex_path = _dir + f'dockings_run{d_run}/{ligand}/{receptor_name}_flex.pdbqt.pdb'
                    ...
                    new_receptor.save_pdb(_dir + f'dockings_run{d_run}/{ligand}/{receptor_name}_receptor.pdb')

                    # stack with top scoring docked pose
                    receptor_ligand = new_receptor.stack(pose)
                    receptor_ligand.save_pdb(_dir + f'dockings_run{d_run}/{ligand}/{receptor_name}_docked2.pdb')

                    # save mdtraj converted pose (only pose)
                    pose.save_pdb(_dir + f'dockings_run{d_run}/{ligand}/{receptor_name}_bestpose.pdb')

                except IndexError:
                    malfunctions.append(f'dockings_run{d_run}/{ligand}/{receptor_name}.pdbqt.pdb')

    np.savetxt(_dir + f'run{d_run}/run{d_run}_failed.log', malfunctions, fmt='%s')
